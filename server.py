...
PUBLIC     = BASE / "public"
MONGO_URI  = os.environ.get("MONGO_URI", "mongodb://localhost:27017/rage")
...
db     = client.get_default_database() if "/" in MONGO_URI.split("@")[-1] else client["rage"]

users  = db["users"]
keys   = db["license_keys"]

# Indexes
users.create_index("username", unique=True)
users.create_index("email",    unique=True)
keys.create_index("key_code",  unique=True)
keys.create_index("user_id")
...
@token_required
def user_me():
    row = users.find_one({"_id": ObjectId(g.cu["id"])})
    u   = ser(row)
...
@token_required
def user_keys():
    rows = keys.find({"user_id": g.cu["id"]}).sort("created_at", DESCENDING)
    out  = []
    for k in rows:
        d = ser(k)
...
def admin_stats():
    ...
    active_keys  = keys.count_documents({"active": True})
    expiring     = keys.count_documents({
        "active": True, "expires_at": {"$ne": None, "$lt": now() + timedelta(days=7)}
    })
    return jsonify(users=total_users, keys=total_keys, active=active_keys, expiring=expiring)
...
def admin_user_role(uid):
    ...
    users.update_one({"_id": ObjectId(uid)}, {"$set": {"role": role}})
    return jsonify(ser(users.find_one({"_id": ObjectId(uid)})))
...
def admin_keys():
    ...
    for k in keys.find().sort("created_at", DESCENDING):
        d = ser(k)
        if d.get("user_id"):
            u = users.find_one({"_id": ObjectId(d["user_id"])}) if len(d["user_id"]) == 24 else None
    ...
    for _ in range(count):
        ...
        try:
            result = keys.insert_one({
                "key_code": code, "product": product,
                "user_id": user_id, "expires_at": expires_at,
                "hwid": None, "hwid_bound_at": None, "last_seen": None,
...
def admin_assign(kid):
    ...
    keys.update_one({"_id": ObjectId(kid)}, {"$set": {"user_id": user_id}})
    return jsonify(ser(keys.find_one({"_id": ObjectId(kid)})))
...
def admin_toggle_active(kid):
    ...
    keys.update_one({"_id": ObjectId(kid)}, {"$set": {"active": active}})
    return jsonify(ser(keys.find_one({"_id": ObjectId(kid)})))
...
def admin_expiry(kid):
    ...
    keys.update_one({"_id": ObjectId(kid)}, {"$set": {"expires_at": exp}})
    return jsonify(ser(keys.find_one({"_id": ObjectId(kid)})))

@app.route("/api/admin/keys/<kid>", methods=["DELETE"])
@admin_required
def admin_delete_key(kid):
    keys.delete_one({"_id": ObjectId(kid)})
    return jsonify(ok=True)
...
def hwid_keys():
    ...
    for k in keys.find().sort("hwid_bound_at", DESCENDING):
        d = ser(k)
        if d.get("user_id"):
            u = users.find_one({"_id": ObjectId(d["user_id"])}) if len(str(d["user_id"])) == 24 else None
            d["assigned_to"] = u["username"] if u else None
        else:
            d["assigned_to"] = None
        out.append(d)
...
@hwid_required
def hwid_reset(kid):
    keys.update_one({"_id": ObjectId(kid)},
                    {"$set": {"hwid": None, "hwid_bound_at": None}})
    return jsonify(ok=True)
...
def blacklist_revoke(kid):
    keys.update_one({"_id": ObjectId(kid)}, {"$set": {"active": False}})
    return jsonify(ok=True)

@app.route("/api/blacklist/keys/<kid>/activate", methods=["PATCH"])
@blacklist_required
def blacklist_activate(kid):
    keys.update_one({"_id": ObjectId(kid)}, {"$set": {"active": True}})
    return jsonify(ok=True)
